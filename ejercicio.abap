...
*    WRITE: / 'Apellido: ',wa_i_alumnos-padron, / 'Nombre', wa_i_alumnos-nombre, / 'Apellido ', wa_i_alumnos-apellido, / 'Fecha de nacimiento: ', wa_i_alumnos-nacimiento, / 'Código postal: ', wa_i_alumnos-postal, / '--------------------------'.
    ...
    MODIFY i_alumnos FROM wa_i_alumnos TRANSPORTING postal.
  ...
  if sy-subrc ne 0.
    WRITE: / 'No existe ese registro'.
    ENDIF.

 "PARA BORRAR REGISTROS DUPLICADOS"
    delete ADJACENT DUPLICATES FROM i_alumnos COMPARING padron.

  "IMPRIMO TODA LA TABLA"
  LOOP at i_alumnos into wa_i_alumnos.
    WRITE: / 'Imprimo mi linea nro', sy-tabix.
    WRITE: / wa_i_alumnos.
